...
import { Medal, Lock, Play, Film, Zap, type LucideIcon } from 'lucide-react'
...
// Lives gravadas (mock — futuramente virá de course_lessons com vimeo_video_id)
type LiveMock = {
  id: string
  title: string
  description: string
...
const MOCK_LIVES: LiveMock[] = [
  {
    id: 'live-1',
    title: 'Agentes de IA na prática: da ideia ao deploy',
    description: 'Live aberta onde construí um agente de prospecção ao vivo com n8n + GPT. Gravada em mar/2026.',
    cover_color: 'bg-gradient-to-br from-red-500 to-pink-500',
    Icon: Film,
    tag: 'Live Gravada',
    duration: '1h 24min',
    date: '22 mar 2026',
    instructor: 'Cigano.agi',
    xp_reward: 50,
    vimeo_id: '76979871',
    is_pro_only: false,
  },
  {
    id: 'live-2',
    title: 'Supabase + RLS: montando backend em 45min',
    description: 'Workshop gravado onde subi autenticação, policies e triggers pra um SaaS do zero. Gravada em abr/2026.',
    cover_color: 'bg-gradient-to-br from-emerald-500 to-teal-600',
    Icon: Zap,
    tag: 'Live Gravada',
    duration: '47 min',
    date: '05 abr 2026',
        ...
        <div className="flex items-end justify-between mt-16 mb-4">
          <div>
            ...
            <p className="text-gray-500 mt-1 text-sm">
              Workshops e aulas ao vivo que já rolaram. Assista on-demand.
            </p>
          </div>
          <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest hidden md:block">
            {MOCK_LIVES.length} gravadas
          </span>
        </div>

        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {MOCK_LIVES.map(live => {
            const locked = live.is_pro_only && !isPro
            return (
              <div
                key={live.id}
                ...
                <div className="p-6 flex-1 flex flex-col justify-between">
                  <div>
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      {live.is_pro_only && (
                        <span className="text-[10px] uppercase tracking-wider font-bold text-white bg-black px-2 py-0.5 rounded-full">
                          Pro
                        </span>
                      )}
                    </div>
                  ...
                  <div className="flex items-center justify-between pt-4 border-t border-gray-50">
                    <div className="flex items-center gap-2 text-xs text-gray-500">
                      <span className="w-6 h-6 rounded-full bg-gray-100 border border-gray-200 flex items-center justify-center text-[10px] font-bold">
                        {live.instructor.slice(0, 2).toUpperCase()}
                      </span>
